...
@media (max-width: 768px) {
 ...
 <script>
    ...
    if (currentPath.includes(".asp")) {
        var cleanPath = currentPath.replace(".asp", "");
...
<script language="JavaScript">
  function updateKontrol(id, checkbox) {
   ...
   window.location.href = "harcirah_liste.asp?id=" + id + "&kontrol=" + kontrol;
  }
 </script> 
    ...
    try {
        ...
        if (data.length > 0) {
            data.forEach(row => {
                ...
                tr.innerHTML = `
                    <td class="tdw" style="${fiyatStili}">
                        <img src="savefiles/${row.resim}" alt="Resim" style="max-width: 100px; max-height: 100px; cursor: pointer;" onclick="openPopup('savefiles/${row.resim}')">
                    </td>
...
async function listele() {
    try {
        // ASP dosyasına istek gönder
        const response = await fetch(`asp/arama_magaza.asp`);
        if (!response.ok) {
            throw new Error(`HTTP Hatası! Durum: ${response.status}`);
        }
        ...
        if (data.length > 0) {
            data.forEach(row => {
                ...
                let fiyatStili = "";
                if (parseFloat(row.fiyat) < 0) {
                    fiyatStili = "style='background-color: yellow; color: black;'"; // Veya istediğiniz stil
                }
...
// Sayfa yüklendiğinde listele fonksiyonunu çağır
document.addEventListener("DOMContentLoaded", function() {
    listele();
});
				...
				<tr>
					<td colspan="16" align="center">
						<form class="form-inline my-2 my-lg-0" onsubmit="event.preventDefault(); filtrele();">
							<input class="form-control mr-sm-2" id="searchInput" type="search" placeholder="Cinsi, Soyad yada Sicil No ile arama yapın..." aria-label="Search">
							<button class="btn btn-outline-success my-4 my-sm-0" type="button" onclick="window.location.href='harcirah_liste.asp'">Temizle</button>
...
<div class="popup" id="imagePopup">
    <div class="popup-inner">
        <button class="close-button" onclick="closeImage()">Kapat</button>
        <img src="" alt="Büyük Resim" class="popup-image" id="popupImage">
    </div>
</div>			
<script language="JavaScript">
